...
# 一些旧版本 transformers (例如 4.30.2) 尚不包含 VitsModel / VitsTokenizer
try:  # 尝试按新版本接口导入
    from transformers import VitsModel, VitsTokenizer  # type: ignore
    _HAS_VITS = True
except Exception:  # noqa
    _HAS_VITS = False
    # 定义轻量占位符，避免 isinstance 报错
    class VitsModel:  # type: ignore
        pass

    class VitsTokenizer:  # type: ignore
        pass
    
    # 记录日志（使用print以免logger尚未初始化）
    print("[New_pipeline_audioldm2] Warning: VitsModel / VitsTokenizer 未在当前 transformers 版本中提供，TTS相关路径将被跳过。")
...
# diffusers 0.25.0 之前/之后版本接口差异兼容
try:
    from diffusers.utils.torch_utils import empty_device_cache, randn_tensor  # type: ignore
except Exception:  # 在0.25.0中可能没有empty_device_cache
    from diffusers.utils.torch_utils import randn_tensor  # type: ignore
    def empty_device_cache():
        """Fallback no-op when diffusers未提供empty_device_cache"""
        pass
...
# 方案A少步数采样器（模块级导入，供 set_sampler 使用）
try:
    from diffusers import DPMSolverMultistepScheduler, UniPCMultistepScheduler  # type: ignore
except Exception:  # noqa
    try:
        from diffusers.schedulers.scheduling_dpmsolver_multistep import (  # type: ignore
...
if is_torch_xla_available():
    try:
        xm = importlib.import_module("torch_xla.core.xla_model")  # type: ignore
        XLA_AVAILABLE = True
    except Exception:
        XLA_AVAILABLE = False
        xm = None  # type: ignore
...
class AudioLDM2Pipeline(DiffusionPipeline):
    ...
    def set_sampler(
        ...
        """Switch scheduler to a fast few-step sampler (DPMSolver++ or UniPC).

        Args:
            sampler: One of {"dpmpp", "dpm++", "dpmsolver", "dpmsolver++", "unipc", "uni-pc"}.
            use_karras_sigmas: Whether to use Karras sigmas (helpful for few-step quality).
            timestep_spacing: Optional override for scheduler's timestep spacing (e.g., "leading", "linspace", "trailing").

        Note:
            Call this before generation. It replaces `self.scheduler` in-place, keeping the original config.
        """
        s = (sampler or "").lower()
        ...
        if s in {"dpmpp", "dpm++", "dpmsolver", "dpmsolver++"}:
            if 'DPMSolverMultistepScheduler' not in globals() or DPMSolverMultistepScheduler is None:  # type: ignore
                raise ImportError("DPMSolverMultistepScheduler is not available in the installed diffusers version.")
            new_sched = DPMSolverMultistepScheduler.from_config(self.scheduler.config)  # type: ignore
            _maybe_set_attr(new_sched, "use_karras_sigmas", use_karras_sigmas)
            _maybe_set_attr(new_sched, "timestep_spacing", timestep_spacing)
            self.scheduler = new_sched
            logger.info(f"Scheduler switched to DPMSolverMultistepScheduler (use_karras_sigmas={use_karras_sigmas}, timestep_spacing={timestep_spacing}).")
        elif s in {"unipc", "uni-pc", "unipc++"}:
            if 'UniPCMultistepScheduler' not in globals() or UniPCMultistepScheduler is None:  # type: ignore
                raise ImportError("UniPCMultistepScheduler is not available in the installed diffusers version.")
            new_sched = UniPCMultistepScheduler.from_config(self.scheduler.config)  # type: ignore
            _maybe_set_attr(new_sched, "use_karras_sigmas", use_karras_sigmas)
            _maybe_set_attr(new_sched, "timestep_spacing", timestep_spacing)
            self.scheduler = new_sched
            logger.info(f"Scheduler switched to UniPCMultistepScheduler (use_karras_sigmas={use_karras_sigmas}, timestep_spacing={timestep_spacing}).")
        else:
            raise ValueError(
                f"Unsupported sampler '{sampler}'. Choose from: dpmpp, dpmsolver, unipc."
            )
    ...
    def generate_language_model(
        ...
        **model_kwargs,
        ...
        """
        # 兼容旧版 transformers：若私有方法不可用，则使用无缓存回退路径
        has_new_api = all(
            hasattr(self.language_model, name)
            for name in ["_get_initial_cache_position", "_update_model_kwargs_for_generation"]
        )

        # 合理默认生成长度
        try:
            default_tokens = int(getattr(self.language_model.config, "max_new_tokens", 8))
        except Exception:
            default_tokens = 8
        max_new_tokens = int(max_new_tokens) if max_new_tokens is not None else default_tokens

        if not has_new_api:
            # 简单逐步前向，不使用缓存（步数较少时影响可忽略）
