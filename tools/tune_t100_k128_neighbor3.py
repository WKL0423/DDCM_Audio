...
def main() -> None:
    root = Path("runs/tune_t100_k128_neighbor3")
    root.mkdir(parents=True, exist_ok=True)

    # Current best from previous round
    baseline = {
        "cfg": "T100K128P2CB4_in900-80",
        "corr": 0.07100893557071686,
        "snr_db": -1.3874041375621584,
        "mel_db_mae": 5.679022312164307,
        "stft_mag_mse": 1.585506796836853,
    }

    T, K, P, CB = 100, 128, 2, 4
    ranges = [(890, 80), (905, 85), (910, 90)]
    ...
    improved_vs_baseline = (
        (best_quality["corr"] > baseline["corr"]) or
        (best_quality["snr_db"] > baseline["snr_db"]) or
        (best_quality["mel_db_mae"] < baseline["mel_db_mae"]) or
        (best_quality["stft_mag_mse"] < baseline["stft_mag_mse"])
    )
    ...
    lines.append("")
    lines.append(f"Improved vs baseline: {bool(improved_vs_baseline)}")
