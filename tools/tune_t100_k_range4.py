...
def main() -> None:
    root = Path("runs/tune_t100_k_range4")
    root.mkdir(parents=True, exist_ok=True)

    T, P, CB = 100, 2, 4
    Ks = [64, 128]
    ranges = [(999, 0), (900, 100)]
    ...
    lines = [
        "| K | t_range | corr | snr_db | mel_db_mae | stft_mse | cum_improve | improved_steps | total_bytes |",
        "|---:|:---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for r in sorted(rows, key=lambda x: (x["corr"], x["snr_db"], -x["mel_db_mae"]), reverse=True):
        lines.append(
            f"| {r['K']} | {r['t_range'][0]}-{r['t_range'][1]} | {r['corr']:.4f} | {r['snr_db']:.4f} | {r['mel_db_mae']:.4f} | {r['stft_mag_mse']:.4f} | {r['cumulative_residual_improvement']:.3f} | {r['improved_steps']}/{r['total_steps']} | {r['bitstream_total_bytes']} |"
        )
