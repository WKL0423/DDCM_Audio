...
def main() -> None:
    root = Path("runs/tune_image_logic")
    root.mkdir(parents=True, exist_ok=True)

    T = 20
    K = 32
    P = 2
    coef_bits_list = [2, 3, 4]
    t_ranges = [(999, 0), (900, 100), (700, 100)]

    rows = []

    for coef_bits in coef_bits_list:
        for t0, t1 in t_ranges:
            out_dir = Path("runs") / f"T={T}_in{t0}-{t1}_K={K}_P={P}_CB={coef_bits}_model=audioldm2-music_audio"
    ...
    json_path = root / "grid_results.json"
    md_path = root / "grid_results.md"
    json_path.write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")

    lines = [
        "| coef_bits | t_range | cum_improve | improved_steps | corr | snr_db | mel_db_mae | stft_mse | total_bytes |",
        "|---:|:---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for r in sorted(rows, key=lambda x: (x["cumulative_residual_improvement"], x["corr"], x["snr_db"]), reverse=True):
        lines.append(
            f"| {r['coef_bits']} | {r['t_range'][0]}-{r['t_range'][1]} | {r['cumulative_residual_improvement']:.3f} | "
            f"{r['improved_steps']}/{r['total_steps']} | {r['corr']:.4f} | {r['snr_db']:.4f} | {r['mel_db_mae']:.4f} | {r['stft_mag_mse']:.4f} | {r['bitstream_total_bytes']} |"
        )
