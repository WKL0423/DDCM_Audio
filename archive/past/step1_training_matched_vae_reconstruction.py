#!/usr/bin/env python3
"""
Step 1: 与训练管道完全匹配的VAE重建测试
基于训练代码的autoencoder.py和配置文件16k_64.yaml创建
确保：
1. 使用16kHz采样率（训练配置）
2. 使用librosa提取mel频谱图（64 bins, fmax=8kHz）
...
class TrainingMatchedVAEReconstructor:
    ...
    def extract_mel_spectrogram(self, audio: np.ndarray) -> torch.Tensor:
        ...
        log_mel = log_mel.transpose(-1, -2)  # [batch, time, freq]
    ...
    def _load_and_encode_audio(self, audio_path: str) -> Tuple[np.ndarray, torch.Tensor, torch.Tensor]:
        ...
        # 限制长度到10秒（训练配置）
        max_length = self.sampling_rate * 10
    ...
    def _save_and_analyze(self, original_audio: np.ndarray, reconstructed_audio: np.ndarray,
        ...
        # 创建输出目录
        output_dir = Path("step_1_training_matched_vae_reconstruction")
        ...
        # 计算mel频谱图差异
        mel_analysis = self._analyze_mel_difference(original_mel, reconstructed_mel)
    ...
    def _analyze_frequency_content(self, original: np.ndarray, reconstructed: np.ndarray) -> Dict:
        ...
        return {
            ...
            "high_freq_ratio": energy_ratio(high_freq_mask),            "spectral_correlation": np.corrcoef(orig_fft, recon_fft)[0, 1]
        }
    
    def _analyze_mel_difference(self, original_mel: torch.Tensor, reconstructed_mel: torch.Tensor) -> Dict:
        """分析mel频谱图差异"""
        # 转为numpy
        ...
        # 确保维度匹配（取最小长度）
        min_time_frames = min(orig_mel_np.shape[0], recon_mel_np.shape[0])
        orig_mel_np = orig_mel_np[:min_time_frames]
        recon_mel_np = recon_mel_np[:min_time_frames]
    ...
    def _generate_analysis_plots(self, original_audio: np.ndarray, reconstructed_audio: np.ndarray,
        ...
        fig.suptitle(f'训练匹配VAE重建分析 - {input_name}', fontsize=16)
        
        # 1. 波形对比
        time_axis = np.arange(len(original_audio)) / self.sampling_rate
        axes[0, 0].plot(time_axis, original_audio, label='原始', alpha=0.7)
        axes[0, 0].plot(time_axis, reconstructed_audio, label='重建', alpha=0.7)
        axes[0, 0].set_title('波形对比')
        ...
        axes[1, 0].semilogy(freqs, orig_fft, label='原始', alpha=0.7)
        axes[1, 0].semilogy(freqs, recon_fft, label='重建', alpha=0.7)
        ...
        axes[1, 1].set_title('频谱误差')
        axes[1, 1].set_xlabel('频率 (Hz)')
        axes[1, 1].set_ylabel('误差幅度')
        axes[1, 1].grid(True)
          # 5. 原始mel频谱图
        orig_mel_np = original_mel.squeeze().cpu().numpy()
        recon_mel_np = reconstructed_mel.squeeze().cpu().numpy()
        ...
        axes[2, 0].set_title('原始Mel频谱图')
        axes[2, 0].set_xlabel('时间帧')
        axes[2, 0].set_ylabel('Mel频段')
        plt.colorbar(im1, ax=axes[2, 0])
        
        # 6. 重建mel频谱图
        im2 = axes[2, 1].imshow(recon_mel_np.T, aspect='auto', origin='lower', cmap='viridis')
        axes[2, 1].set_title('重建Mel频谱图')
        axes[2, 1].set_xlabel('时间帧')
        axes[2, 1].set_ylabel('Mel频段')
        plt.colorbar(im2, ax=axes[2, 1])
        
        plt.tight_layout()
        
        # 保存图表
        plot_path = output_dir / f"{input_name}_training_matched_analysis_{timestamp}.png"
...
def main():
    ...
    reconstructor = TrainingMatchedVAEReconstructor()
      # 测试音频文件路径
    test_audio_path = "vae_vs_diffusion_comparison/original_input.wav"  # 使用现有的测试音频
